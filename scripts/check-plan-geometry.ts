...
{
  // Auto-numbering: Bedroom, Bedroom 2, Bedroom 3
  const rooms: PlanRoom[] = [];
  for (let i = 0; i < 3; i += 1) {
    const type: PlanRoomType = "bedroom";
    const name = nextAutoRoomName(rooms, type);
    rooms.push({
      ...rectRoom(`b${i}`, i * 140, 0, 120, 120),
      type,
      category: roomTypeCategory(type),
      name,
    });
  }
  assert(rooms[0]!.name === "Bedroom", "auto-name: first Bedroom", rooms[0]!.name);
  assert(rooms[1]!.name === "Bedroom 2", "auto-name: Bedroom 2", rooms[1]!.name);
  assert(rooms[2]!.name === "Bedroom 3", "auto-name: Bedroom 3", rooms[2]!.name);
}

{
  // Custom name survives type change (same rules as setRoomType in room-ops)
  let room: PlanRoom = {
    ...rectRoom("r1", 0, 0, 100, 100),
    type: "bedroom",
    category: "living",
    name: "Bedroom",
  };
  assert(
    isAutoGeneratedRoomName(room.name, room.type),
    "custom: starts as auto name",
  );
  room = { ...room, name: "Kids Suite" };
  assert(
    !isAutoGeneratedRoomName(room.name, room.type),
    "custom: typed name is not auto",
  );
  const nextType: PlanRoomType = "office";
  ...
  room = {
    ...
    name: keepName
      ? room.name
      : nextAutoRoomName([room], nextType, room.id),
  };
  assert(
    room.name === "Kids Suite",
    "custom name survives type change",
    room.name,
  );
  assert(room.type === "office", "type updated to office");
}

{
  // Undo/redo round trip via pure history helpers over document snapshots
  const start: FloorGeometry = {
    ...createEmptyFloorGeometry("hist"),
    rooms: [rectRoom("a", 0, 0, 144, 120)],
  };
  let hist = createGeometryHistory(start);

  const renamed: FloorGeometry = {
    ...start,
    rooms: start.rooms.map((r) => ({ ...r, name: "Den", type: "bedroom" })),
  };
  hist = historyPush(hist, renamed);

  const moved: FloorGeometry = {
    ...renamed,
    rooms: renamed.rooms.map((r) => ({
      ...r,
      polygon: r.polygon.map((p) => ({ x: p.x + 24, y: p.y })),
      labelAnchor: { x: r.labelAnchor.x + 24, y: r.labelAnchor.y },
    })),
  };
  hist = historyPush(hist, moved);

  const withStairs: FloorGeometry = {
    ...moved,
    stairs: [
      {
        id: "s1",
  ...
  hist = historyPush(hist, withStairs);
